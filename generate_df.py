...
def generate_df_2D( model_path, output_path, options ):
    ...
    ranges = np.linspace(BORDES[0], BORDES[1], options['width'])
    i_1, i_2 = np.meshgrid( ranges, ranges )
    samples = np.concatenate( np.array([ i_1.flatten()[...,None], i_2.flatten()[...,None] ]), axis=1 )

    gradients = np.zeros((SAMPLES, 2))
    pred_distances = evaluate( model, samples, device=device_torch, gradients=gradients )
    pred_grad_norm = np.linalg.norm( gradients , axis=1 ).reshape((SAMPLES, 1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mesh_path', metavar='path/to/mesh.ply', type=str,
    ...
    parser.add_argument('--twod', action='store_true', help='2d reconstruction')

    args = parser.parse_args()
    d = vars(args)
    if args.twod:
        d['hidden_layer_nodes'] = [64,64,64,64]
        d['activation']='sine'
        generate_df_2D( args.model_path, args.output_path, d)
